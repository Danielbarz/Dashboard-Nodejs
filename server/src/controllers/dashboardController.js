...
export const getReportTambahan = async (req, res, next) => {
  try {
    ...
    // Reuse the richer JT aggregation logic from reportController (parent/child, status buckets)
    ...
    // Top 3 usia per witel (parent only)
    const topByWitel = {}
    projectRows.forEach(row => {
    ...
    // Top 3 usia per PO (parent + child context)
    const topByPo = {}
    projectRows.forEach(row => {
    ...
    // Preview data: highest usia first (limit to avoid huge payload)
    const previewData = await prisma.spmkMom.findMany({
      where: applyDateFilter(whereClause),
      orderBy: { usia: 'desc' },
      take: 200
    })
