...
export const getFilterOptions = async (req, res, next) => {
  try {
    ...
    const productOptions = [...new Set(rows.map((r) => r.product).filter(Boolean))].sort()
    const witelList = [...new Set(rows.map((r) => r.witel).filter(Boolean))].sort()
    const subTypeList = [...new Set(rows.map((r) => r.subType).filter(Boolean))].sort()
    const branchList = [...new Set(rows.map((r) => r.branch || NULL_BRANCH_LABEL))].sort()
    const witelBranchMap = [...new Set(rows.map((r) => `${r.witel}|||${r.branch || NULL_BRANCH_LABEL}`))]
      .map((key) => {
        const [nama_witel, telda] = key.split('|||')
        return { nama_witel, telda }
      })
      .sort((a, b) => a.nama_witel.localeCompare(b.nama_witel) || a.telda.localeCompare(b.telda))
...
export const getReportTambahan = async (req, res, next) => {
  try {
    const data = await computeReportTambahanData(req.query)
    successResponse(res, data, 'Report Tambahan data retrieved successfully')
  } catch (error) {
    next(error)
  }
}
...
export const getReportDatin = async (req, res, next) => {
  try {
    const data = await computeReportDatinData(req.query)
    successResponse(res, data, 'Report Datin data retrieved successfully')
  } catch (error) {
    next(error)
  }
}
...
export const getReportAnalysis = async (req, res, next) => {
  try {
    const data = await computeReportAnalysisData(req.query)
    successResponse(res, data, 'Report Analysis data retrieved successfully')
  } catch (error) {
    next(error)
  }
}
...
export const getReportHSI = async (req, res, next) => {
  try {
    const data = await computeReportHsiData(req.query)
