      ...
      tooltip: {
        callbacks: {
          label: (ctx) => {
            ...
            const item = tooltipLookup[po]?.[rankIndex]
            const nama = item?.nama_project || '-'
            const usia = item?.usia ?? 0
            const witelInfo = item?.childWitel ? ` — ${item.childWitel}` : ''
            return `Rank ${rankIndex + 1}: ${usia} hari — ${nama}${witelInfo}`
          }
        }
      }
    }
