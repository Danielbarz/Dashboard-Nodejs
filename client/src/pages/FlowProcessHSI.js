                ...
                <div className="absolute left-0 mt-1 w-full bg-white border border-gray-300 rounded-md shadow-lg max-h-60 overflow-y-auto z-50">
                    {options.map((option) => (
                        <div key={option} className="flex items-center p-2 hover:bg-gray-100 cursor-pointer" onClick={() => toggleOption(option)}>
                            ...
                            <span className="ml-2 text-sm text-gray-700 select-none">{option}</span>
                        </div>
                    ))}
                </div>
            )}
...
const HeaderStep = ({ title, color = "bg-gray-800", isLast }) => (
    <div className="relative flex-1 min-w-[120px]">
        <div className={`${color} text-white font-bold h-12 flex items-center justify-center px-4 md:px-8 text-xs md:text-sm text-center relative z-10 shadow-md transition-all hover:brightness-110`}
            style={{ clipPath: isLast ? 'polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%, 100% 100%, 0% 100%, 15px 50%)' : 'polygon(0% 0%, calc(100% - 15px) 0%, 100% 50%, calc(100% - 15px) 100%, 0% 100%, 15px 50%)', marginLeft: '-12px', paddingLeft: '2rem' }}>
            {title}
        </div>
    </div>
);
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col items-center">
                <div className="text-2xl md:text-3xl font-extrabold text-gray-800 leading-none">
                    {loading ? '...' : (count?.toLocaleString() || 0)}
                </div>
                {percent && !loading && <div className="text-xl md:text-2xl font-bold text-green-600 mt-1">{percent}</div>}
            </div>
        </div>
    );
};
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0">
                ...
                {percent && !loading && <span className={`text-lg md:text-xl font-bold ${highlight ? 'text-red-500' : 'text-blue-500'}`}>({percent})</span>}
            </div>
        </div>
    );
};
...
const TreeCard = ({ title, count, total, color = "bg-white", borderColor = "border-gray-300", textColor = "text-gray-800", onClick, loading }) => {
    ...
    return (
        ...
        >
            ...
            <div className="flex flex-col items-center">
                <div className={`text-xl font-extrabold ${textColor} leading-tight`}>
                    {loading ? '...' : (count?.toLocaleString() || 0)}
                </div>
                {percent && !loading && <div className={`text-lg font-bold ${textColor} opacity-90 mt-1`}>{percent}</div>}
            </div>
        </div>
    );
};
    ...
    const handleExportDetail = async () => {
        ...
        try {
            ...
            if (error.response && error.response.status === 401) {
                ...
                alert("Terjadi kesalahan sistem saat membuat Excel.");
            } else {
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                        <div>
                            ...
                            <div className="relative z-40">
                                <DatePicker selectsRange={true} startDate={startDate} endDate={endDate} onChange={(u) => setDateRange(u)} isClearable={true} placeholderText="Pilih Rentang" className="w-full border-gray-300 rounded-md text-sm shadow-sm p-2.5" dateFormat="dd/MM/yyyy" />
                            </div>
                        </div>
                        <div>
                        ...
                        <div className="flex gap-2">
                            <button onClick={loadStats} className="bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold py-2.5 px-4 rounded shadow w-full">Terapkan</button>
                        </div>
                    </div>
                </div>
