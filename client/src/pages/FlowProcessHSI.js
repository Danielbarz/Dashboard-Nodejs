...
const FlowProcessHSI = () => {
  ...
  const handleExportDetail = () => {
    ...
    alert(`Exporting data for: ${activeCategory}`)
  }

  // CALCULATIONS (RASIO)
  const psRePercent =
    flowStats?.ps_re_denominator > 0 ? ((flowStats.ps_count / flowStats.ps_re_denominator) * 100).toFixed(2) : 0

  const psPiPercent =
    flowStats?.ps_pi_denominator > 0 ? ((flowStats.ps_count / flowStats.ps_pi_denominator) * 100).toFixed(2) : 0

  const complyCount = flowStats?.comply_count || 0
  const complyDenominator = flowStats?.ps_count || 1
  const complyPercent = flowStats?.ps_count > 0 ? ((complyCount / complyDenominator) * 100).toFixed(2) : 0
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0">
          ...
          {percent && (
            <span className={`text-lg md:text-xl font-bold ${highlight ? 'text-red-500' : 'text-blue-500'}`}>
              ({percent})
            </span>
          )}
        </div>
      ...
      <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 border-b border-gray-300 pb-4">
          ...
          <p className="text-sm text-gray-500 mt-1 flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse"></span> Last Update:{' '}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <div className="flex gap-2">
              ...
              {(startDate || selectedWitels.length > 0 || selectedBranches.length > 0) && (
                <button onClick={resetFilter} className="bg-white border border-gray-300 text-gray-700 text-sm font-bold py-2.5 px-4 rounded shadow">
                  Reset
                </button>
              )}
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-bold text-gray-800">
                    Detail Data: <span className="text-blue-600">{activeCategory}</span>
                  </h3>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`px-2 py-1 rounded-full text-xs font-bold ${
                              item.kelompok_status === 'PS'
                                ? 'bg-green-100 text-green-800'
                                : item.kelompok_status && item.kelompok_status.includes('CANCEL')
